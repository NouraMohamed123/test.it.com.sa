...
<div class="breadcrumb">
    <h1>{{ __('translate.candidates_List') }}</h1>
    <ul>
        <li><a href="/employees">{{ __('translate.candidates') }}</a></li>
        <li>{{ __('translate.candidates') }}</li>
    </ul>
</div>
    ...
    <div class="col-md-12">
        <div class="card text-left">
            <div class="card-header text-right bg-transparent">
                @can('employee_add')
                <a class="btn btn-primary btn-md m-1" href="{{route('candidates.create')}}"><i
                ...
                <div class="table-responsive">
                    <table id="employee_list_table" class="display table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td></td>
                                <td @click="selected_row( {{ $candidate->id}})"></td>
                                <td>{{$candidate->jop->job_title}}</td>
                                <td>
                                    <div class="ul-widget-app__profile-pic">
                                        <a href="{{ asset('assets/images/avatar/'.$candidate->cv) }}"  target="_blank" >{{ $candidate->cv}}</a>
                                    </div>
                                </td>
                                    {{-- @can('employee_details')
                                    <a href="/candidates/{{$candidate->id}}" class="ul-link-action text-info"
...
<td>
                                    @can('employee_edit')
                                    <a href="/candidates/{{$candidate->id}}/edit" class="ul-link-action text-success"
                                    ...
                                    @can('employee_delete')
                                    <a @click="Remove_Employee( {{ $candidate->id}})"
            ...
            delete_selected() {
                ...
                swal({
                    ...
                    buttonsStyling: false
                        ...
                        axios
                        .post("/candidates/delete/by_selection", {
